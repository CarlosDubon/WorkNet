...
function focusable( element, isTabIndexNotNaN ) {
	...
	if ( "area" === nodeNombre ) {
		...
		img = $( "img[usemap=#" + mapNombre + "]" )[0];
		return !!img && visible( img );
	}
	return ( /input|select|textarea|button|object/.test( nodeNombre ) ?
		!element.disabled :
		"a" === nodeNombre ?
...
$.widget = function( name, base, prototype ) {
	var fullNombre, existingConstructor, constructor, basePrototype,
	...
	if ( existingConstructor ) {
		$.each( existingConstructor._childConstructors, function( i, child ) {
			...
			$.widget( childPrototype.namespace + "." + childPrototype.widgetNombre, constructor, child._proto );
		});
...
$.widget.bridge = function( name, object ) {
	var fullNombre = object.prototype.widgetFullNombre || name;
...
$.Widget.prototype = {
	...
	destroy: function() {
		...
		this.element
			.unbind( this.eventNombrespace )
			// 1.9 BC for #7810
			// TODO remove dual storage
			.removeData( this.widgetNombre )
			.removeData( this.widgetFullNombre )
			// support: jquery <1.6.3
			// http://bugs.jquery.com/ticket/9413
			.removeData( $.camelCase( this.widgetFullNombre ) );
		this.widget()
			.unbind( this.eventNombrespace )
			.removeAttr( "aria-disabled" )
			.removeClass(
				this.widgetFullNombre + "-disabled " +
				"ui-state-disabled" );

		// clean up events and states
		this.bindings.unbind( this.eventNombrespace );
	...
	_setOption: function( key, value ) {
		...
		if ( key === "disabled" ) {
			this.widget()
				.toggleClass( this.widgetFullNombre + "-disabled ui-state-disabled", !!value )
	...
	_on: function( suppressDisabledCheck, element, handlers ) {
		...
		$.each( handlers, function( event, handler ) {
			...
			var match = event.match( /^(\w+)\s*(.*)$/ ),
				eventNombre = match[1] + instance.eventNombrespace,
				selector = match[2];
			if ( selector ) {
				delegateElement.delegate( selector, eventNombre, handlerProxy );
			} else {
				element.bind( eventNombre, handlerProxy );
			}
		});
	},

	_off: function( element, eventNombre ) {
		eventNombre = (eventNombre || "").split( " " ).join( this.eventNombrespace + " " ) + this.eventNombrespace;
		element.unbind( eventNombre ).undelegate( eventNombre );
	},
...
$.widget("ui.mouse", {
	...
	_mouseInit: function() {
		...
		this.element
			.bind("mousedown."+this.widgetNombre, function(event) {
				return that._mouseDown(event);
			})
			.bind("click."+this.widgetNombre, function(event) {
	...
	_mouseDestroy: function() {
		this.element.unbind("."+this.widgetNombre);
		if ( this._mouseMoveDelegate ) {
			$(document)
				.unbind("mousemove."+this.widgetNombre, this._mouseMoveDelegate)
				.unbind("mouseup."+this.widgetNombre, this._mouseUpDelegate);
		}
	},
	...
	_mouseDown: function(event) {
		...
		var that = this,
			...
			elIsCancel = (typeof this.options.cancel === "string" && event.target.nodeNombre ? $(event.target).closest(this.options.cancel).length : false);
		...
		$(document)
			.bind("mousemove."+this.widgetNombre, this._mouseMoveDelegate)
			.bind("mouseup."+this.widgetNombre, this._mouseUpDelegate);
...
(function () {
	var testElement, testElementParent, testElementStyle, offsetLeft, i,
		body = document.getElementsByTagNombre( "body" )[ 0 ],
...
$.widget( "ui.button", {
	...
	_create: function() {
		this.element.closest( "form" )
			.unbind( "reset" + this.eventNombrespace )
			.bind( "reset" + this.eventNombrespace, formResetHandler );
		...
		this.buttonElement
			...
			.attr( "role", "button" )
			.bind( "mouseenter" + this.eventNombrespace, function() {
				if ( options.disabled ) {
					return;
				}
				if ( this === lastActive ) {
			...
			.bind( "mouseleave" + this.eventNombrespace, function() {
				if ( options.disabled ) {
					return;
				}
				$( this ).removeClass( activeClass );
			})
			.bind( "click" + this.eventNombrespace, function( event ) {
				if ( options.disabled ) {
					event.preventDefault();
					event.stopImmediatePropagation();
				}
		...
		if ( toggleButton ) {
			this.element.bind( "change" + this.eventNombrespace, function() {
				that.refresh();
			});
		}

		if ( this.type === "checkbox" ) {
			this.buttonElement.bind( "click" + this.eventNombrespace, function() {
				if ( options.disabled ) {
					return false;
				}
			});
			...
			this.buttonElement
				.bind( "mousedown" + this.eventNombrespace, function() {
					if ( options.disabled ) {
						return false;
					}
					$( this ).addClass( "ui-state-active" );
				...
				.bind( "mouseup" + this.eventNombrespace, function() {
					if ( options.disabled ) {
						return false;
					}
					$( this ).removeClass( "ui-state-active" );
				})
				.bind( "keydown" + this.eventNombrespace, function(event) {
					if ( options.disabled ) {
						return false;
					}
				...
				.bind( "keyup" + this.eventNombrespace + " blur" + this.eventNombrespace, function() {
					$( this ).removeClass( "ui-state-active" );
				});
...
+function ($) {
  ...
  function transitionEnd() {
    ...
    for (var name in transEndEventNombres) {
      if (el.style[name] !== undefined) {
        return { end: transEndEventNombres[name] }
      }
    }
...
+function ($) {
  ...
  Modal.prototype.measureScrollbar = function () { // thx walsh
    ...
    scrollDiv.classNombre = 'modal-scrollbar-measure'
...
(function ($) {
  ...
  function TagsInput(element, options) {
    ...
    this.isSelect = (element.tagNombre === 'SELECT');
  ...
  TagsInput.prototype = {
    ...
    add: function(item, dontPushVal) {
      ...
      if (typeof item === "string" && this.$element[0].tagNombre === 'INPUT') {
    ...
    build: function(options) {
      ...
      if (self.options.itemValue === defaultOptions.itemValue) {
        if (self.$element[0].tagNombre === 'INPUT') {
  ...
  function makeOptionItemFunction(options, key) {
    if (typeof options[key] !== 'function') {
      var propertyNombre = options[key];
      options[key] = function(item) { return item[propertyNombre]; };
    }
  }
...
(function($) {
        ...
        return function hightlight(o) {
            ...
            function hightlightTextNode(textNode) {
                ...
                if (match = regex.exec(textNode.data)) {
                    wrapperNode = doc.createElement(o.tagNombre);
                    o.classNombre && (wrapperNode.classNombre = o.classNombre);
    ...
    var Input = function() {
        ...
        _.mixin(Input.prototype, EventEmitter, {
            ...
            _onKeydown: function onKeydown($e) {
                var keyNombre = specialKeyCodeMap[$e.which || $e.keyCode];
                this._managePreventDefault(keyNombre, $e);
                if (keyNombre && this._shouldTrigger(keyNombre, $e)) {
                    this.trigger(keyNombre + "Keyed", $e);
                }
            },
            _onInput: function onInput() {
                this._checkInputValue();
            },
            _managePreventDefault: function managePreventDefault(keyNombre, $e) {
            ...
            _shouldTrigger: function shouldTrigger(keyNombre, $e) {
                var trigger;
                switch (keyNombre) {
                  case "tab":
    ...
    var Dataset = function() {
        ...
        function Dataset(o) {
            ...
            if (o.name && !isValidNombre(o.name)) {
        ...
        Dataset.extractDatasetNombre = function extractDatasetNombre(el) {
            ...
            getDatumForSuggestion: function getDatumForSuggestion($el) {
                ...
                if ($el.length) {
                    datum = {
                        ...
                        datasetNombre: Dataset.extractDatasetNombre($el)
                    };
                }
                return datum;
            },
    ...
    var Typeahead = function() {
        ...
        _.mixin(Typeahead.prototype, {
            ...
            _onCursorMoved: function onCursorMoved() {
                ...
                this.eventBus.trigger("cursorchanged", datum.raw, datum.datasetNombre);
            },
            ...
            _autocomplete: function autocomplete(laxCursor) {
                ...
                if (hint && query !== hint && isCursorAtEnd) {
                    ...
                    this.eventBus.trigger("autocompleted", datum.raw, datum.datasetNombre);
                }
            },
            _select: function select(datum) {
                ...
                this.input.setInputValue(datum.value, true);
                this._setLanguageDirection();
                this.eventBus.trigger("selected", datum.raw, datum.datasetNombre);
                this.dropdown.close();
...
(function ($, undefined) {
     ...
     * @param formatter
     */
    function checkFormatter(formatter, formatterNombre) {
        if ($.isFunction(formatter)) return true;
        if (!formatter) return false;
        if (typeof(formatter) === 'string') return true;
        throw new Error(formatterNombre +" must be a string, function, or falsy value");
    }
   ...
   */
    ...
    AbstractSelect2 = clazz(Object, {
        ...
        init: function (opts) {
            ...
            this.containerEventNombre= this.containerId
                .replace(/([.])/g, '_')
                .replace(/([;&,\-\.\+\*\~':"\!\^#$%@\[\]\(\)=>\|])/g, '\\$1');
        ...
        prepareOpts: function (opts) {
            ...
            if (element.get(0).tagNombre.toLowerCase() === "select") {
        ...
        // abstract
        opening: function() {
            var cid = this.containerEventNombre,
                scroll = "scroll." + cid,
                resize = "resize."+cid,
    ...
    SingleSelect2 = clazz(AbstractSelect2, {
        ...
        prepareOpts: function () {
            ...
            if (opts.element.get(0).tagNombre.toLowerCase() === "select") {
