...
class Categoria {
        ...
        $valores = '"'.$cat.'","'.$idCuenta.'"'; 
         ...
         if($this->validarCategoriaUnica($cat)){
            $resultado = $bd->insertarRegistro($tabla, $columnas, $valores);
             $utilidades->mostrarMensaje('¡La categoría se agregó correctamente!');
            $plantilla->verPagina('');
         }else{
            $utilidades->mostrarMensaje('¡Lo sentimos, la categoría existe, por favor intente de nuevo.');
            $plantilla->verPagina('');
            return 0;
    }
}
 private function validarCategoriaUnica($Categoria) {
    ...
    public function eliminarCategoria($id){
        ...
        if($result)
            $utilidades->mostrarMensaje('La categoría se eliminó correctamente');
